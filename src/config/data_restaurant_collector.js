...
async function main() {
  ...
  if (restaurants) {
    try {
      ...
      console.log(
        `Dados dos restaurantes salvos com sucesso em '${filePath}'.`
      );
    } catch (error) {
      console.error("Erro ao salvar os dados dos restaurantes:", error);
    }
  } else {
    console.log("Falha ao pontos restaurantes.");
  }
}
